...
function Navbar() {
  ...
  return (
    <header className="h-[72px] bg-black border-b border-white/10 flex items-center justify-between px-6 shrink-0 z-50">

      {/* ── Sol: Logo ve Proje Adı ── */}
      <div className="flex items-center gap-3 w-auto md:w-[260px] shrink-0">
        <div className="relative shrink-0">
          <div className="w-10 h-10 rounded-xl border border-white/20 bg-white/5 flex items-center justify-center shadow-lg shadow-white/5">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span className="absolute bottom-0 left-0 w-full h-[2px] bg-white shadow-[0_0_8px_rgba(255,255,255,0.4)] rounded-t-full" />
              )}
            </NavLink>
          )
        })}
      ...
      <div className="flex items-center gap-3 shrink-0">
        {/* Arama */}
        <div className="flex items-center gap-2 bg-white/5 hover:bg-white/10 rounded-lg px-3 py-2 border border-white/10 transition-colors duration-200 group">
          ...
          />
          <kbd className="text-[10px] text-white/40 bg-white/10 border border-white/20 px-1.5 py-0.5 rounded font-mono hidden xl:inline-block">
            ⌘K
          </kbd>
        </div>

        {/* Bildirimler */}
        <button className="relative p-2 rounded-lg hover:bg-white/10 transition-all duration-200 text-white/60 hover:text-white group">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="relative">
              <div className="w-8 h-8 rounded-lg border border-white/20 bg-white/5 flex items-center justify-center shadow-md shadow-white/5">
            ...
            <ChevronDown className={`w-3.5 h-3.5 text-white/50 group-hover:text-white transition-all duration-200 hidden sm:block ${isDropdownOpen ? 'rotate-180' : ''}`} />
          </button>

          {/* Dropdown Menu */}
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-black border border-white/10 rounded-xl shadow-xl shadow-black/50 py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
